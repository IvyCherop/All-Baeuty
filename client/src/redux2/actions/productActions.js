  ...
  dispatch({ type: PRODUCT_SEARCH_REQUEST, payload: searchTerm });
    try {
      const { data } = await axios.get(
        `http://localhost:5000/api/v1/products/search?search_query=${searchTerm}`
      );
...
export const categoryProducts = (category) => async (dispatch) => {
  ...
  try {
    const { data } = await axios.get(
      `http://localhost:5000/api/v1/products/category?category=${category}`
    );
    console.log(data);
...
export const updateItem = (product) => async (dispatch, getState) => {
  ...
  try {
    const { data } = await axios.put(
      `http://localhost:5000/api/v1/products/find/${product.productId}`,
